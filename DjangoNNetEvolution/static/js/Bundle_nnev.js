...
var Vector2 = (function () {
    ...
    Vector2.fromCartesian = function (x, y) {
...
var Circle = (function () {
    ...
    Circle.prototype.draw = function (ctx, scale) {
        ...
        ctx.arc(this.position.x * scale, this.position.y * scale, this.radius * scale, 0, Math.PI * 2);
        ctx.fill();
    };
...
var Tree = (function () {
    ...
    Tree.prototype.draw = function (ctx, scale) {
        if (scale === void 0) { scale = 1; }
        this._body.draw(ctx, scale);
    };
    return Tree;
}());
Tree.initialEnergy = 0.5;
Tree.radius = 1;
exports.Tree = Tree;
var Animal = (function () {
    ...
    Animal.prototype.resetObservedTrees = function () {
        var i = 0;
        var len = this.observedTrees.length;
        while (i < this.observedTrees.length)
    ...
    Animal.prototype.tryToObserveTree = function (tree) {
        var where = this.inWhichSectors(tree.position);
        if (!isNaN(where[0])) {
    ...
    Animal.prototype.inWhichSectors = function (point) {
        return [
            this.inWhichSector(this.eye0, point),
            this.inWhichSector(this.eye1, point)
        ];
    };
    Animal.prototype.inWhichSector = function (eye, point) {
        var relPos = point.add(eye.position.scale(-1)).rotate(-eye.angle + Animal.fieldOfViewAngle / 2);
        if (relPos.r < Animal.fieldOfViewR &&
            0 < relPos.theta &&
            relPos.theta < Animal.fieldOfViewAngle) {
            var segmentAngle = Animal.fieldOfViewAngle / Animal.fieldOfViewSegments;
...
var World = (function () {
    ...
    World.prototype.addUpdateListener = function (listener) {
        this.updateListeners.push(listener);
    };
    Object.defineProperty(World.prototype, "numberOfTrees", {
        get: function () {
    ...
    Object.defineProperty(World.prototype, "numberOfAnimals", {
        get: function () {
    ...
    World.prototype.addObject = function (object) {
        ...
        if (object instanceof Animal) {
            ...
            if (this.animals == null) {
                this.animals = new List_1.ListNode(object);
            }
            else {
            ...
            this._numberOfTrees++;
            if (this.trees == null) {
                this.trees = new List_1.ListNode(object);
            }
            else {
                var t = new List_1.ListNode(object);
    ...
    World.prototype.spawnAnimalsFromParent = function (parent) {
        var numberToSpawn = this.initialNumberOfAnimals - this.numberOfAnimals;
        for (var i = 0; i < numberToSpawn / 2; i++) {
            var a = parent.giveAncestor(0.25);
            a.position = Vector2.fromCartesian(this.width * Math.random(), this.height * Math.random());
            this.addObject(a);
        }
        this.spawnAnimals();
    };
    World.prototype.spawnTrees = function () {
        for (var i = this.numberOfTrees; i < this.maxNumberOfTrees / 4; i++) {
    ...
    World.prototype.spawnAdditionalEntities = function (timePassed) {
        ...
        var probOfNewTree = Math.min(0.1, timePassed / 1000 * World.treesGrowthRate);
        for (var t = this.trees; t != null; t = t.next) {
    ...
    World.prototype.addNewRandomTree = function () {
        this.addObject(new Tree(Vector2.fromCartesian(Math.random() * this.width, Math.random() * this.height)));
    };
    World.prototype.addNewRandomAnimal = function () {
        this.addObject(new Animal(Vector2.fromCartesian(Math.random() * this.width, Math.random() * this.height), Net_1.Net.randomNet(Animal.netLayersLength), 2 * Math.PI * Math.random()));
    };
    World.prototype.removeAnimalNode = function (n) {
        if (n === this.animals) {
    ...
    World.prototype.removeTreeNode = function (n) {
        if (n === this.trees) {
            this.trees = n.remove();
        }
        else {
...
var ListNode = (function () {
    ...
    ListNode.prototype.toStringForward = function () {
        var node = this;
        var s = "";
        while (node != null) {
            s += "(" + node.data + ")";
    ...
    ListNode.prototype.toStringBackward = function () {
        var node = this;
        var s = "";
        while (node != null) {
            s = "(" + node.data + ")" + s;
...
var Net = (function () {
    ...
    function Net(layersSizes, parameters, sigmoid) {
        ...
        var length = layersSizes[i] * layersSizes[i + 1];
        this.transforms[i] =
    ...
    Net.randomNet = function (layersSizes) {
        var totalLength = 0;
        var i = 0;
        for (; i < layersSizes.length - 2; i++) {
        ...
        var parameters = new Array(totalLength);
        for (var i_1 = 0; i_1 < totalLength; i_1++) {
            parameters[i_1] = (Math.random() * 2 - 1) * 1 / Math.sqrt(layersSizes[0]);
        }
